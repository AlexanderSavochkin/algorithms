...
// Decimation in time domain version of FFT
// This implementation destroys the input data
template <typename Tcontainer, typename Tvalue>
void fft_time_decimation(Tcontainer& input_data, Tcontainer& output_data, unsigned int log2_size, int direction)
{
	const complex<Tvalue> I = complex<Tvalue>(Tvalue(0.0), Tvalue(1.0));
	Tcontainer* current_data = &input_data, *next_data = &output_data;
	...
	for (int d = log2_size - 1; d >= 0; --d)
	{
		...
		for (int c = 0; c < buffer_size; c+=N*2)
		{
			for (int i = 0; i < N; ++i)
			{
				(*next_data)[c + i] = (*current_data)[c + 2*i];
				(*next_data)[c + i + N] = (*current_data)[c + 2*i + 1];
			}
		}
	...
	// Composing spectra from partial results
	for (int d = 0; d < log2_size; ++d)
	{
		// Partial signal length
		unsigned N = 1 << d;
